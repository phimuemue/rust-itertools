...
fn extrema_set() {
    ...
    assert_eq!(None::<Option<u32>>.iter().min_set::<Vec<_>>(), None);
    assert_eq!(None::<Option<u32>>.iter().max_set::<Vec<_>>(), None);

    assert_eq!(Some(1u32).iter().min_set::<Vec<_>>(), Some(vec![&1]));
    assert_eq!(Some(1u32).iter().max_set::<Vec<_>>(), Some(vec![&1]));

    let data = vec![Val(0, 1), Val(2, 0), Val(0, 2), Val(1, 0), Val(2, 1)];

    let min_set = data.iter().min_set::<Vec<_>>().unwrap();
    ...
    let max_set = data.iter().max_set::<Vec<_>>().unwrap();
